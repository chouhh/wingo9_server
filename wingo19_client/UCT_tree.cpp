...
void Node::create_Children(int *CandidateMoves , int num_moves , int c , int hn )
{
    Node* next_node = NULL;
      ...
      for(int i = 0 ; i < num_moves ; i++ )
      {
          ...
          if(Child == NULL)
          {
            Node* temp = new Node(c,level+1,CandidateMoves[i]);
            Child = temp;
          }
          else
          {
...
int Node::get_NodesValues(int* wins,int* visits)
{
	Node* next = Child;
