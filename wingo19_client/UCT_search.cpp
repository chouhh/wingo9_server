...
/*int get_BestMove(Node *r)
{
	...
	if (r->get_Child() != NULL)
	{
		...
		if (best != NULL)
		{
			if ((float) best->get_Wins() / best->get_Visits() == 0)
			{
				result = RESIGN;
			}
			else
...
void play_Simulation(Board *b, Node *P_root)
{
	...
	get_NodeSequence(P_root, b, Node_Sequence, &Max_Level);//取出節點的選擇順序
	win_color = b->play_RandomGame();//模擬
	update_Tree(Node_Sequence, Max_Level, win_color);//更新
}
		...
		*Max_Level = temp->get_Level();
		...
		else //代表這點並非leaf，但不排除是 end game
		{
			if (!temp->has_Child()) //若temp沒有子點
			{
				if (b->get_LegalNum() != 0)
				{
					if (b->get_hand_num() < Open_Step_Limit)
					{
						int moves[81] = {0};
						int num_moves = b->select_urgency_moves(moves);
						if (num_moves > 0)
