...
int gtp_com_play(string sn)
{
	...
	if (hit_book && (hn <= Open_Step_Limit))
	{
		...
		else
		{
			...
			if (hit_joseki)
			{
				num_joseki_moves = pattern->get_joseki_moves(c,b->board,joseki_moves);

				if (num_joseki_moves > 0)
				{
					if (num_joseki_moves == 1)
					{
						m = joseki_moves[0];
						fout << "Hit joseki success!" << m << endl;
					}
					else
					{
						m = find_nearest_joseki(num_joseki_moves,joseki_moves,b->last_move);
						fout << "Hit joseki success!" << m << endl;
					}
				}
				else
